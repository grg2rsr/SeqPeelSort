...
for i, unit in enumerate(Config['general']['units']):
    ...
    if config['adaptive_threshold'] == True:
        ...
        for i, seg in enumerate(Blk.segments):
            SpikeTrain, = select_by_dict(seg.spiketrains, unit=unit,
                                         kind='thresholded')
            frate_at_spikes.append(calc_frate_at_spikes(SpikeTrain))
            spike_amps.append(SpikeTrain.waveforms.max(axis=1))

        frate_at_spikes = sp.concatenate(frate_at_spikes,
                                         axis=0)[:, 0] * frate_at_spikes[0].units
        ...
        if Config['general']['fig_format'] is not None:
            ...
            plot_amp_reduction(pfit, frate_at_spikes, spike_amps, Config, unit,
                               save=outpath)
        ...
        if pfit is not None:
            for i, seg in enumerate(Blk.segments):
                ...
                frate = ele.statistics.instantaneous_rate(SpikeTrain,
                                                          kernel='auto',
                                                          sampling_period=SpikeTrain.sampling_period)

                # calc adaptive bounds
                adap_bound_lower, adap_bound_upper = calc_adaptive_threshold(
                    frate, pfit, config['bounds'])
                adap_bound_lower.annotate(unit=unit,
                                          kind='adaptive_threshold_lower')
                adap_bound_upper.annotate(unit=unit,
                ...
                SpikeTrain_adap = adaptive_threshold(SpikeTrain,
                                                     adap_bound_lower,
                                                     adap_bound_upper)
...
if Config['general']['fig_format'] is not None:
    for i, seg in enumerate(Blk.segments):
        ...
        plot_spike_detect(seg, Config, save=outpath,
...
for i, unit in enumerate(Config['general']['units']):
    config = Config[unit]
    Templates_sim[unit] = simulate_Templates(Templates_cleaned[unit],
                                             n_sim=config['n_sim'],
                                             n_comp=config['n_comp'])[0]
...
def tm_run(AnalogSignal, templates_sim, config):
    ...
    SpikeTrain_TM, Score_TM = spike_detect_on_TM(Scores_TM, config['wsize'],
                                                 percentile=config['tm_percentile'],
                                                 thresh=config['tm_thresh'])
...
for seg in tqdm(Blk.segments, desc='template matching segment'):
    ...
    for i, unit in enumerate(Config['general']['units']):
        ...
        if i == 0:
            ...
            Asig, = select_by_dict(seg.analogsignals,
                                   kind='V_peeled after ' + previous_unit)

        # peeling tm runs
        V_peeled, V_recons, SpikeTrain_TM, Score_TM = tm_run(Asig,
                                                             Templates_sim[unit],
                                                             config)
...
if Config['general']['fig_format'] is not None:
    for i, seg in enumerate(Blk.segments):
        outpath = os.path.join(exp_name+'_results', 'plots',
                               'TM_result_'+str(i))
        plot_TM_result(seg, Config, zoom=None, save=outpath)
        outpath = os.path.join(exp_name+'_results', 'plots',
                               'TM_result_zoomed_'+str(i))
        plot_TM_result(seg, Config, zoom=Config['general']['zoom'],
                       save=outpath)
...
if output_format == 'csv':
    ...
    for i, seg in enumerate(Blk.segments):
        for j, st in enumerate(seg.spiketrains):
            outpath = os.path.join(exp_name+'_results',
                                   os.path.splitext(os.path.basename(data_path))[0]+"_unit_"+st.annotations['unit']+"_trial_"+str(i)+"_sorted.csv")
            print_msg("writing output to "+outpath)
